...
module BulkLoad
  ...
  # 1. post /bulk_load/guess - stateless non REST API to return guess result [NEW]
  # 2. post /bulk_load/preview - stateless non REST API to return preview result [NEW]
  ...
  def bulk_load_guess(job)
    # retry_request = true
    res = api { post('/v3/bulk_load/guess', job.validate.to_json) }
    unless res.ok?
  ...
  def bulk_load_preview(job)
    # retry_request = true
    res = api { post('/v3/bulk_load/preview', job.validate.to_json) }
    unless res.ok?
